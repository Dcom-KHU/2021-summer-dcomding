	...
	while(!funcStack.empty()){
        ...
        if(usedCyclePtr[club]<eachClubCycles[club].size()){
            while(usedCyclePtr[club]<eachClubCycles[club].size()){
                if(existLastCourse.count(club)){
                    string tempClub = eachClubCycles[club][usedCyclePtr[club]].size()?eachClubCycles[club][usedCyclePtr[club]][0]:club;
                    if(tempClub.length() == nextClubs.top().length() && tempClub>nextClubs.top()) break;
                    else if(tempClub.length() > nextClubs.top().length()) break;
                }
